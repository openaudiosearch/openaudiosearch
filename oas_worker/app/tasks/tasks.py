...
def download(opts):
    ...
    target_path = file_path(f"download/{target_name}")
    ...
    with requests.get(url, stream=True) as res:
        ...
        logger.info(f"Downloading {url} to {target_path} ({pretty_bytes(total_size)})")
...
def asr(args, opts):
    ...
    model_base_path = config.model_path or os.path.join(config.storage_path, "models")
