...
def run_with_args(worker, args):
    ...
    log.info(f"active jobs: {list(worker.tasks)}")
    try:
...
def run(worker):
    ...
    epilog = f"Registered jobs: {list(worker.tasks)}"
    parser = argparse.ArgumentParser(description=description, epilog=epilog)
    parser.add_argument('--single', type=str, help='Run a single job of this typ', metavar="JOB")
    #  parser.add_argument('--dashboard', action='store_true', help='Enable the task dashboard on port {DASHBOARD_PORT}')
