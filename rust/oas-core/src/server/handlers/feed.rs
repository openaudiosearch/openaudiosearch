use oas_common::{types::Feed, util, Record, TypedValue};
...
pub async fn post_feed(
    ...
    body: Json<Feed>,
) -> Result<Json<TxSuccess>, AppError> {
    ...
    match feed.validate() {
        Ok(_) => {
            ...
            let result = state.store.table::<Feed>().put_one(feed).await?;
            Ok(Json(result))
        }
...
pub async fn get_feed(
    ...
    state: &rocket::State<State>,
    id: String,
) -> Result<Json<Record<Feed>>, AppError> {
    let feed = state.db.get_record(&Feed::guid(&id)).await?;
    Ok(Json(feed))
}
...
pub async fn delete_feed(
    ...
    let result = state.db.delete_record(&Feed::guid(&id)).await?;
    Ok(Json(result))
}
...
pub async fn get_feeds(
    _user: AdminUser,
    state: &rocket::State<State>,
) -> Result<Json<Vec<Record<Feed>>>, AppError> {
